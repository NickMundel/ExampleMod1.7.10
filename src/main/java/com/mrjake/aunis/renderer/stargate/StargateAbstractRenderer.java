...
import com.mrjake.aunis.loader.texture.Texture;
import com.mrjake.aunis.loader.texture.TextureLoader;
import com.mrjake.aunis.renderer.BlockRenderer;
...
import com.mrjake.aunis.renderer.stargate.StargateRendererStatic.QuadStrip;
...
public abstract class StargateAbstractRenderer<S extends StargateAbstractRendererState> extends TileEntitySpecialRenderer<StargateAbstractBaseTile> {
		...
		if (rendererState != null) {
			...
			if (shouldRender(rendererState)) {
				...
				for (Map.Entry<BlockPos, IBlockState> entry : getMemberBlockStates(te.getMergeHelper(), rendererState.facing).entrySet()) {
					...
					if (BaseUtils.getWorldBlockState(getWorldObj(), absolutePos).getBlock().isReplaceable(getWorldObj(), absolutePos.getX(), absolutePos.getY(), absolutePos.getZ()))
						BlockRenderer.render(getWorldObj(), pos, entry.getValue(), absolutePos);
				}
	...
	protected boolean shouldRender(S rendererState) {
		IBlockState state = BaseUtils.getWorldBlockState(getWorldObj(), rendererState.pos);
...
//		rendererState.vortexState = EnumVortexState.FULL;
		...
		float gateWait = getWorldObj().getTotalWorldTime() - rendererState.gateWaitStart;
		...
		float tick = (float) (getWorldObj().getTotalWorldTime() - kawooshStart + partialTicks);
		float mul = 1;
...
//								mul = 0.945f;
						...
						float arg2 = (float) ((getWorldObj().getTotalWorldTime() - stateChange + partialTicks) / 3f) - 1.0f;
						...
						if (arg2 < StargateRendererStatic.eventHorizonRadius+0.1f) {
							...
							if (getWorldObj().getTotalWorldTime() - stateChange - 9 > 7) {
...
//								clearChevrons(stateChange + 9 + 7);
					...
					if ( (getWorldObj().getTotalWorldTime() - rendererState.gateWaitClose) > 35 ) {
						float arg2 = (float) ((getWorldObj().getTotalWorldTime() - (rendererState.gateWaitClose+35) + partialTicks) / SPEED_FACTOR / 2f);
...
//					ModelLoader.bindTexture(ModelLoader.getTexture("stargate/event_horizon_by_mclatchyt_2_unstable.jpg"));
	...
	protected void renderEventHorizon(double partialTicks, boolean white, Float alpha, boolean backOnly, float mul) {
		float tick = (float) (getWorldObj().getTotalWorldTime() + partialTicks);
