...
import com.mrjake.aunis.stargate.network.SymbolPegasusEnum;
...
import static com.mrjake.aunis.gui.element.GuiHelper.isPointInRegion;
...
public class StargateContainerGui extends GuiContainer implements TabbedContainerInterface {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		for (int i=7; i<11; i++)
			((Tab.SlotTab) container.getSlot(i)).updatePos();
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
		...
		for (int i=0; i<3-container.gateTile.getSupportedCapacitors(); i++) {
			func_146110_a(guiLeft+151 - 18*i, guiTop+40, 24, 175, 16, 16, 512, 512);
		}

		for (int i=container.gateTile.getPowerTier(); i<4; i++)
			func_146110_a(guiLeft+10+39*i, guiTop+61, 0, 168, 39, 6, 512, 512);
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		fontRendererObj.drawString(I18n.format("gui.stargate.capacitors"), 112, 29, 4210752);

		String energyPercent = String.format("%.2f", energyStored/(float)maxEnergyStored * 100) + " %";
		fontRendererObj.drawString(energyPercent, 168-fontRendererObj.getStringWidth(energyPercent)+2, 71, 4210752);
		...
		if (isPointInRegion(10, 61, 156, 6, mouseX, mouseY)) {
			...
			drawHoveringText(power, mouseX-guiLeft, mouseY-guiTop, fontRendererObj);
		}
	}
