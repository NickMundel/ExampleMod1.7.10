...
public class TabAddress extends Tab {
	...
	public void render(FontRenderer fontRenderer, int mouseX, int mouseY) {
		...
		Gui.func_146110_a(guiLeft+currentOffsetX+105, guiTop+defaultY+86, 6, 174, 18, 18, textureSize, textureSize);
		...
		if (isVisible() && gateTile.getStargateAddress(symbolType) != null) {
			...
			Gui.func_146110_a(guiLeft+currentOffsetX+97, guiTop+defaultY+86+(18-progress), 0, 174+(18-progress), 6, progress, textureSize, textureSize);
	...
	public SlotTab createSlot(Slot slot) {
		return new SlotTab(slot, (slotTab) -> {
			slotTab.xDisplayPosition = currentOffsetX + 106;
			slotTab.yDisplayPosition = defaultY + 87;
		});
	}
