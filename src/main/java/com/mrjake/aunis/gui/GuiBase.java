...
public class GuiBase extends GuiScreen {
	...
	protected void translateToCenter() {
		GL11.glTranslatef((width - imageWidth)/2f, (height - imageHeight)/2f, 0);
	}
...
//		y *= 10;
	...
	protected void drawVerticallCenteredString(String text, int x, int y, int color) {
		int len = fontRendererObj.getStringWidth(text);
	...
	protected void drawTextBox(GuiTextField tf) {
		int x = tf.xPosition - 2;
		int y = tf.yPosition - 2;
		int x2 = tf.xPosition + tf.width;
		int y2 = tf.yPosition + tf.height;
	...
	protected GuiTextField createTextField(int x, int y, int maxLength, String defaultText) {
		...
		GuiTextField tf = new GuiTextField(fontRendererObj, x1+3, y1-1, getBottomRightInside(false)-x1-6, 10);
