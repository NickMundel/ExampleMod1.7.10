package com.mrjake.aunis.chunkloader;

import com.mrjake.aunis.Aunis;
import com.mrjake.aunis.util.ChunkPos;
...
//			Aunis.info("Creating ticket for "+world.provider);
	...
	public static void forceChunk(World world, ChunkPos chunk) {
        Aunis.LOG.debug("Forcing chunk " + chunk + ", in world: " + world.provider);

		Ticket ticket = requestTicket(world);
		ForgeChunkManager.forceChunk(ticket, new ChunkCoordIntPair(chunk.x, chunk.z));
	...
	public static void unforceChunk(World world, ChunkPos chunk) {
		Ticket ticket = requestTicket(world);
		ForgeChunkManager.unforceChunk(ticket, new ChunkCoordIntPair(chunk.x, chunk.z));
	...
	public static void onWorldUnload(WorldEvent.Unload event) {
		worldTicketMap.remove(event.world);
	}
}
