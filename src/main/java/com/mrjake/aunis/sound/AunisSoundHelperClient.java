...
public class AunisSoundHelperClient {
    ...
    private static Map<BlockPos, Map<SoundPositionedEnum, ISound>> positionedSoundRecordsMap = new HashMap<>();

    private static PositionedSoundRecord getSoundRecord(SoundPositionedEnum soundEnum, BlockPos pos) {
        return new PositionedSoundRecord(soundEnum.resourceLocation, 0.5f, 1.0f, pos.getX()+0.5f, pos.getY()+0.5f, pos.getZ()+0.5f);
    }

    public static void playPositionedSoundClientSide(BlockPos pos, SoundPositionedEnum soundEnum, boolean play) {
        Map<SoundPositionedEnum, ISound> soundRecordsMap = positionedSoundRecordsMap.get(pos);
        ...
        ISound soundRecord = soundRecordsMap.get(soundEnum);
        ...
        if (play) {
            ...
            soundHandler.stopSound(soundRecord);
        }
    }
}
