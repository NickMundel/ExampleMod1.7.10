...
import com.mrjake.aunis.block.stargate.StargateAbstractMemberBlock;
...
import com.mrjake.aunis.util.BlockHelpers;
...
import static com.mrjake.aunis.stargate.EnumMemberVariant.CHEVRON;
import static com.mrjake.aunis.stargate.EnumMemberVariant.RING;
...
public abstract class StargateAbstractMergeHelper {
	...
	public List<BlockPos> getAbsentBlockPositions(IBlockAccess world, BlockPos basePos, EnumFacing facing, EnumMemberVariant variant) {
		...
		return blocks.stream()
				...
				.filter(pos -> !matchMember(BaseUtils.getWorldBlockState(world, pos)))
	...
	public StargateAbstractBaseTile findBaseTile(IBlockAccess blockAccess, BlockPos memberPos, EnumFacing facing) {
		...
		for (BlockPos.MutableBlockPos pos : BlockPos.getAllInBoxMutable(globalBox.getMinBlockPos(), globalBox.getMaxBlockPos())) {

            if (matchBase(BaseUtils.getWorldBlockState(blockAccess, pos))) {
				return (StargateAbstractBaseTile) blockAccess.getTileEntity(pos.toImmutable().getX(), pos.toImmutable().getY(), pos.toImmutable().getZ());
			}
		}
