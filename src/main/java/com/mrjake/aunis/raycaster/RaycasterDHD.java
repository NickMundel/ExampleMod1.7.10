...
public class RaycasterDHD extends Raycaster {
	...
	public boolean onActivated(World world, BlockPos pos, EntityPlayer player) {
		float rotation = BaseUtils.getWorldBlockState(world, pos).getValue(AunisProps.ROTATION_HORIZONTAL) * -22.5f;

		button = -1;

		return super.onActivated(world, pos, player, rotation);
	}
	...
	@Override
	protected boolean brbCheck(List<Ray> brbRayList, Vec3d lookVec, EntityPlayer player, BlockPos pos) {
		if (button == -1) {
